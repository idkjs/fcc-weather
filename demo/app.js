...
/***/ (function(module, exports) {
	...
	function createWeatherElements(createElement, createWeatherTemplate, locations) {
	    return locations.filter(location => location.id !== null && location.city !== undefined).map(createWeatherTemplate).map(createElement);
	}

	function processSearchResponse(response) {
	    clearElement('localWeather');
	    const elements = response.length > 0 ? createWeatherElements(createElement, createWeatherTemplate, response) : [console.log("error in processSearchResults")];
	...
	function getDetails(lat, lon) {
	    ...
	    const url = `http://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&units=metric&APPID=${APPID}`;
